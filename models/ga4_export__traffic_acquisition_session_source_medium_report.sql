...
traffic_acquisition_report as (
    ...
    select
        ...
        count(distinct case when event_name = 'user_engagement' then session_id end)/ nullif(count(distinct session_id),0) as engagement_rate_comparison,
        avg(case when event_name = 'user_engagement' then 1 else 0 end) as engagement_rate,
        count(unique_event_id) as event_count,
        (count(unique_event_id)) / nullif(count(distinct session_id),0) as events_per_session,
