...
first_user_source_medium as (
    ...
    select
        ...
        row_number() over (partition by events.user_pseudo_id {{ fivetran_utils.partition_by_source_relation(package_name='ga4_export') }} order by 
        ...
        end, events.event_timestamp) as row_num
    from {{ ref('stg_ga4_export__event') }} as events
    inner join user_first_data as ufd
        on events.user_pseudo_id = ufd.user_pseudo_id
        and events.event_timestamp = ufd.first_event_timestamp
        and events.source_relation = ufd.source_relation
